...
def findVert(name: str, new: "Graph") -> "Vertex":
    verts = new.vertices
    for i in verts:
        if name == i.label:
            return i
    return Vertex(new, name)


def isEdge(hl: str, tl: str, g: "Graph"):
    for e in g.edges:
        ehl = e.head.label
        etl = e.tail.label
        if (ehl == hl and etl == tl) or (etl == hl and ehl == tl):
            return True
    return False


def handleGraph(new: "Graph", d: dict, name: str = "") -> "Graph":
    for k in d.keys():
        for v in d[k]:
            if not isEdge(k, v, new):
                new.add_edge(Edge(findVert(f'{name}{k}', new), findVert(f'{name}{v}', new)))
    return new
...
class Graph(object):
    ...
    def __add__(self, other: "Graph") -> "Graph":
        ...
        """
        G = Graph(False)
        handleGraph(G, createDict(self), "G - ")
        handleGraph(G, createDict(other), "H - ")
