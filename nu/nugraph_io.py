...
from nugraph import Graph, Edge
...
def load_graph(f: IO[str], graph_class=Graph, read_list: bool = False) -> Union[
    Tuple[List[Graph], List[str]], Graph]:
    """
...
def write_dot(graph: Graph, f: IO[str], directed=False):
    ...
    for v in graph:
        ...
        if hasattr(v, 'colortext'):
            options += 'fontcolor="' + v.colortext + '",'
        if hasattr(v, 'rgb') and v.rgb is not None:
            options += 'style=filled,fillcolor="#' + v.rgb + '",'
